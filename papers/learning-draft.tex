...
The effects of these informational frictions are substantial: one-third of the patients would be referred to a different specialist under perfect information, and there would be small but meaningful improvements in patient outcomes.
...
These are large differences relative to the national average failure rate of 9.5 percentage points.
...
The results from the model indicate that there are substantial losses due to informational frictions. With perfect information about specialist quality, our counterfactual simulations show that about one-third of patients would be referred to different specialists. The rate of negative outcomes would decrease by 5\%, in relative terms, and episode spending would fall similarly. 
% and episode spending would fall by xxx\%, in relative terms. 
Because the simulations account for specialist capacity constraints and habit persistence in referrals, these changes could reasonably be attributed to informational frictions. %[The estimated capacity constraints are nontrivial, and the effects of habit persistence are substantial... xxx] %TODO fix this
...
The results under ``resetting familiarity'' are summarized in Figure \ref{fig:cf_full}, where we find that \reallocationMeanMyopic{}\% (32.1\%) of existing referrals would be reallocated in the myopic (forward-looking) model. This represents a substantial change in demand for many specialists. The changes in health outcomes are more modest, which is not surprising given the relatively small average failure rate. Under full information, the probability of a negative outcome falls by about \healthFXMeanMyopic{} percentage points relative to the average failure rate of around 0.09, making the relative reduction in the failure rate about 5\%.
%% TODO (Seth): see embedded comment about dollar value: 0.3 percentage points on a base of 90\%
These results include adjustments for changes in congestion, as discussed in Section \ref{sec:estimation}, and are relatively similar across different values of $\eta$. The results under ``existing familiarity'' are similar and appear in Figure \ref{fig:cf_full_current}.
% TODO: Note the nationwide reduction in failures per year. Based on 150k surgeries per year? Is that just in our limited sample?

Collectively, our counterfactuals indicate that informational frictions have substantial effects on the allocation of referrals. With perfect information, about one-third of patients would be referred to a different specialist. In the existing informational environment, PCPs learn slowly about quality differences among specialists---partly because opportunities for learning (e.g., negative outcomes) are somewhat rare. Familiarity also plays a role, maintaining referral patterns regardless of patient outcomes. The congestion effect among patients referred to the same specialists further reduces the ability of the market to allocate patients to better performing specialists. Consequently, in the existing environment, the referring physician's response to patient outcomes is relatively muted. %particularly any subsequent effects on patient health. 
\iffalse
