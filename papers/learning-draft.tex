...
As reflected in Table \ref{tab:sum-pairs} and Figure \ref{fig:network}(a), most PCPs have experience referring patients to between 3 and 5 unique specialists per year and around \cumlNetworkSize{} unique specialists over our entire 2013-2018 estimation period. For context, we observe an average of nearly \specsPerHRRYear{} orthopedic surgeons performing a major joint replacement per market per year, or \meanSpecsPerHRR{} specialists per market over the full estimation period. So the average network size of \cumlNetworkSize{} specialists over the estimation period implies that an average PCP remains unfamiliar with over 85\% of available specialists in their market. This suggests a lack of experimentation with other specialists and therefore a potential opportunity for quality and efficiency improvements if PCPs could better allocate their referrals to higher quality and more efficient specialists in their market. 
...
We summarize the results from the stacked event study in Figure \ref{fig:event}, where we find a statistically significant reduction of approximately \eventStudyCoef{} referrals per quarter per specialist from affected PCPs (i.e., those PCPs whose patients experienced a bad outcome) relative to unaffected PCPs (i.e., PCPs who refer to the same specialists but whose patients did not experience a bad outcome). Our specification normalizes $\lambda_{\tau=-1}=0$. In the underlying data for this event study, the average quarterly referral rate for treated pairs in the pre-failure periods is \eventStudyBaseRate{}. Our estimates therefore imply that a failure event leads to an approximate \eventStudyPctReduction{}\% reduction in referrals from a PCP to the specialist with the failure. Event study results for individual failure cohorts are consistent with out stacked estimates and presented in the supplemental appendix.
