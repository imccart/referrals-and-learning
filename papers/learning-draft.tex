...
\begin{abstract}
\singlespacing
% TODO: Revise framing here and conclusion. Our counterfactuals primarily measure losses due to informational frictions, while accounting for familiarity and capacity constraints / congestion. We also have some good explanations for why learning is slow in the current environment.
\noindent In many areas of care, primary care physicians (PCPs) greatly influence a patient's choice of specialist, but how well do PCPs learn about specialist quality, and how does this affect market allocations and patient outcomes? In this paper, we study PCP referrals to specialists, using the population of orthopedic procedures for Medicare fee-for-service beneficiaries from 2008 through 2018. We document substantial heterogeneity in specialist quality and cost within geographic markets, and we present an event study showing that PCPs adjust their referrals specifically based on the outcomes of their own patients. We then employ a structural learning model to measure learning and efficiency in this market. In the model, PCPs update their beliefs about specialist quality based on the outcomes of their patients. The model also accounts for habit persistence and capacity constraints, which are important limitations on potential reallocations when there is better information about quality. In both analyses, we find that PCPs respond modestly to the outcomes of their referred patients. % OLD: ; however, PCPs remain slow to learn about quality differences among specialists in their market, instead relying more on prior relationships in governing referrals. This suggests there is scope to improve market efficiency if PCPs can learn more quickly and better tailor referral decisions on specialist quality. 
The effects of these informational frictions are substantial: about one-quarter of patients would be referred to a different specialist under perfect information, with meaningful improvements in patient outcomes. We further show that existing referral relationships encode quality information, amplifying the gains from improved quality transparency.
% TODO (revise the abstract, here is some other possible language): We use a structural learning model to measure the effects of uncertainty in physician referrals and to simulate possible reallocations when there is better information about quality. We find that one-third of these patients would be referred to a different specialist in the absence of informational frictions, with small but meaningful improvements in patient quality of care.
...
% ALT: ...a defining feature of markets for healthcare goods and services
...
% TODO: I changed the above numbers b/c we don't have to state the 25th and 75th percentiles across markets of the IQRs. We can include more markets in "most markets" for example 10th to 90th.
...
% TODO: Seth -- why do we do this? Would the results hold up if we have the same sample of specialists?
...
\item Identify the PCP(s) whose patient(s) experienced a failure following a referral to specialist $j$ in $q_j(1)$.\footnote{Typically only one patient experiences a failure from a given specialist in a given quarter, but there are cases where specialists are associated with multiple failures in the same quarter.} Denote this PCP (or set of PCPs) as type $k=1$, which constitutes the treatment group for this analysis. Similarly, identify the other PCPs whose patients do not experience a failure with specialist $j$ over the relevant time period, and denote this set of PCPs as type $k=0$, which is the control group.

\item Calculate the means by PCP type of the number of referrals per PCP per quarter to specialist $j$, removing the referral(s) that resulted in failure. Denote the mean quarterly patients referred to specialist $j$ from PCP type $k$ by $\bar{r}_{jkt}$.\footnote{We collapse the data by ``PCP type'' primarily to ease the computational burden. Results are unchanged if we instead estimate Equation \ref{eqn:eventstudy} at the PCP-specialist-quarter level, allowing for both specialist and PCP fixed effects.}
\end{enumerate}
...
%TODO (seth): Add discussion, emphasizing how this is consistent with learning from your own patients. Indeed, this is very well targeted at precisely that variation. Perhaps wait until after presenting in Arkansas to articulate this.
We present the main results from the stacked event study in Figure \ref{fig:event}, where we find a statistically significant reduction of approximately \eventStudyCoef{} referrals per quarter per specialist from treated PCPs (i.e., those whose patients experienced a bad outcome) relative to other PCPs (i.e., those who refer to the same specialists but whose patients did not experience a bad outcome).\footnote{The results for the separate failure cohorts are consistent with these stacked estimates and are presented in the supplemental appendix.} 
% Our specification normalizes $\lambda_{\tau=-1}=0$. [SETH: I think this is clear enough from the equation.]
This is compelling evidence of learning specifically based on the outcomes of referred patients, because any changes in supply from the specialists with a bad outcome, or changes in demand from other PCPs, are absorbed by the specialist-by-quarter fixed effects that capture the control observations.
To interpret the magnitude of the effect, the average quarterly referral rate, among treated PCP-specialist pairs with at least one referral in the pre-failure quarters, is \eventStudyBaseRateCond{}. Our estimates therefore imply that a failure event leads to an approximately \eventStudyPctReductionCond{}\% reduction in referrals from the PCP whose patient experienced the failure to the specialist with whom the failure occurred.\footnote{The unconditional average quarterly referral rate is \eventStudyBaseRate{}, which includes many quarters with zero referrals for a given PCP-specialist pair. Using this as the denominator would yield a \eventStudyPctReduction{}\% relative reduction, but this overstates the behavioral response since PCPs cannot reduce referrals below zero.} 
...
\centering
\begin{minipage}[h]{6in}
\caption[Gradient of Health Effects]{Health Effects of Full Information by Estimated Learning Parameter\footnote{Figure depicts the mean change in successes per 10,000 patients under each counterfactual, binned into ten patient-weighted quantiles of the estimated learning parameter $\hat\alpha$ within each HRR ($\hat\alpha > 0.001$). Marker size reflects the total patients in the bin. Results shown for $\eta=1$; corresponding figures for $\eta=5$ appear in the supplemental appendix.}}
\centering
\begin{tabular}{cc}
\multicolumn{2}{c}{\textit{Myopic behavior}} \\
...
\centering
\begin{minipage}[h]{6in}
\caption[Gradient of Spending Effects]{Episode Spending Effects of Full Information by Estimated Learning Parameter\footnote{Figure depicts the mean change in episode spending under each counterfactual, binned into ten patient-weighted quantiles of the estimated learning parameter $\hat\alpha$ within each HRR ($\hat\alpha > 0.001$). Marker size reflects the total patients in the bin. Results shown for $\eta=1$; corresponding figures for $\eta=5$ appear in the supplemental appendix.}}
\centering
\begin{tabular}{cc}
\multicolumn{2}{c}{\textit{Myopic behavior}} \\
Existing familiarity & Reset familiarity \\
...
\centering
\begin{minipage}[h]{6in}
\caption[caption]{Counterfactual Changes with Quality-Maximizing Referrals\footnote{Figure depicts the share of referrals reallocated and the resulting change in probability of success under a counterfactual in which each PCP's belief is replaced with the true specialist success rate, specialist fixed effects and accumulated familiarity are set to zero, and $\alpha$ within each HRR is calibrated to match the larger of the distance or congestion utility components. Results are averaged across individuals within an HRR and presented as a histogram across HRRs. Results shown for $\eta=1$; corresponding figures for $\eta=5$ appear in the supplemental appendix.}}
\centering
\begin{tabular}{cc}
\multicolumn{1}{c}{\textit{Myopic}} & \multicolumn{1}{c}{\textit{Forward-Looking}} \\
\multicolumn{2}{c}{\textit{Reallocation of referrals}} \\
