...
% TODO: Do we want to say anything about how many people are affecte, or how much is spent each year? Or leave that to the background section (because readers already believe this is important enough).
...
Because the simulations control for capacity constraints, as well as habit persistence in referrals, these changes could reasonably be attributed to informational frictions. %[The estimated capacity constraints are nontrivial, and the effects of habit persistence are substantial... xxx] %TODO fix this
...
We summarize the results from the stacked event study in Figure \ref{fig:event}, where we find a statistically significant reduction of approximately \eventStudyCoef{} referrals per quarter per specialist from affected PCPs (i.e., those PCPs whose patients experienced a bad outcome) relative to unaffected PCPs (i.e., PCPs who refer to the same specialists but whose patients did not experience a bad outcome). Our specification normalizes $\lambda_{\tau=-1}=0$. Among treated PCP-specialist pairs with at least one referral in the pre-failure quarters, the average quarterly referral rate is \eventStudyBaseRateCond{}. Our estimates therefore imply that a failure event leads to an approximate \eventStudyPctReductionCond{}\% reduction in referrals from a PCP to the specialist with the failure.\footnote{The unconditional average quarterly referral rate is \eventStudyBaseRate{}, which includes many quarters with zero referrals for a given PCP-specialist pair. Using this as the denominator would yield a \eventStudyPctReduction{}\% relative reduction, but this overstates the behavioral response since PCPs cannot reduce referrals below zero.} Event study results for individual failure cohorts are consistent with out stacked estimates and presented in the supplemental appendix.
