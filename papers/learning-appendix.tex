...
% TODO: Estimate without the term $\frac{\rho \eta}{\eta + e_{ijt}}$.  Instead define the variable $m$ as just $\frac{y_{ijt}}{\eta + e_{ijt}}$. (Just to see if the conjecture is right.)
...
We use an iterative procedure proposed in % TODO: add reference
which makes it possible to employ standard estimation commands designed for multinomial logits where all terms are linear in parameters.
...
% TODO: add reference — proves this algorithm converges to the MLE of $\ln \alpha$ under standard regularity conditions.
...
\] 
The standard error then follows as % TODO: add reference
\[
    SE(\hat \alpha) = \exp(\ln \alpha_*) SE(\hat \pi_*)
\]
because $\hat \alpha = \exp(\ln \alpha_*) = \exp(\ln \alpha_* + \hat \pi_* - 1)$ and asymptotically we only need to consider the variation in $\hat \pi_*$ . % TODO: add reference (Lemma 2)
