...
\section{First-Stage Diagnostics for Congestion}
\label{app:first-stage}

The congestion parameter $\gamma$ is estimated via two-stage least squares, regressing specialist fixed effects on total patient volume and HRR fixed effects, with distance-based predicted patient counts as the excluded instrument. Table \ref{tab:first-stage} reports the pooled first-stage results. The first-stage coefficient is strongly significant, and the F-statistic on the excluded instrument exceeds conventional thresholds for both values of $\eta$.

Figure \ref{fig:fs-scatter} presents partial scatter plots of actual versus predicted patient counts after residualizing on HRR fixed effects and a time-period indicator, confirming a strong positive relationship between the instrument and endogenous patient volume. Figure \ref{fig:fs-fstat} further shows the distribution of HRR-level first-stage F-statistics, weighted by patient volume.
