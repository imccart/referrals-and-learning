...
Re-estimating the full structural pipeline and recomputing counterfactuals for every alternative specification is computationally prohibitive. As a more tractable diagnostic, we examine how the estimated quality response, $\alpha$, varies across alternative specifications of the first-step multinomial logit. Because the counterfactual reallocations and health effects scale directly with $\alpha$ in each market, stability of $\alpha$ across specifications implies stability of our counterfactual conclusions.
...
Here, we discuss how risk adjustment does not materially affect our estimates of specialist quality. First, our analytic sample is heavily concentrated in a single, low-complexity DRG. Table~\ref{tab:drg-distribution} reports the distribution of admissions across DRGs. DRG 470 (major joint replacement of lower extremity, without MCC) accounts for 86.1 percent of the 2.92 million events, with the remaining 14 percent spread across 17 procedure codes. The combination of elective admission and physician referral as sample inclusion criteria selects a planned, lower-complication population in which case-mix variation across specialists is limited.

Second, raw and risk-adjusted specialist quality are highly correlated. We construct a risk-adjusted measure of specialist quality via indirect standardization, regressing the patient-level 90-day failure indicator on age (quadratic), sex, race, DRG, and the Charlson Comorbidity Index, then computing each specialist's observed minus expected failure rate plus the overall sample mean. The Pearson correlation between raw and risk-adjusted specialist quality is 0.84, indicating that the within-specialist sorting captured by our raw measure largely survives explicit case-mix adjustment.

Third, the estimated quality response is invariant to either case-mix sensitivity. Table~\ref{tab:coefficients-adj} reports the distribution of $\hat\alpha$ under two re-estimations of the first-step multinomial logit. The first restricts the estimation sample to DRG 470 only, and the second swaps the risk-adjusted specialist-quality measure into the belief prior. The mean of $\hat\alpha$ moves from 0.192 in the baseline (Table~\ref{tab:coefficients}) to 0.216 when restricting to DRG 470 only, and to 0.192 with the risk-adjusted quality measure. The upper-tail percentiles are similarly stable.

Because the analytic sample is concentrated in a single elective DRG, the raw and risk-adjusted quality measures are tightly correlated, and the estimated quality response is invariant to either case-mix sensitivity, we retain the raw specialist-quality measure throughout. Building risk adjustment into the structural model would require re-deriving the beta-binomial belief-updating apparatus around a continuous, risk-adjusted signal and reconstructing the Gittins index that governs the forward-looking decision problem. Given the evidence above, that complexity is not warranted.
...
\footnotesize
\begin{minipage}[h]{6in}
\caption[DRG Distribution]{\textbf{DRG Distribution of the Analytic Sample}\footnote{Distribution of admissions across DRG codes in the analytic sample (2008--2018, major joint replacement procedures with elective admission and physician referral). ``Events'' is the number of admissions in each DRG; ``Share'' is the share of all admissions.}}
\centerline{%
\resizebox{\linewidth}{!}{\input{results/tables/drg-distribution.tex}}
}
...
\centerline{%
    \begin{tabular}{lrrrrrr}
        \input{results/tables/MNL_Specs_Adj.tex}
    \end{tabular}
}
