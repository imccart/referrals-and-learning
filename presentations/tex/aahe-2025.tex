...
\date[]{AAHE Research Conference \\ September 2025}
...
\bit
    ...
    \item Gong (2025) considers a new surgical procedure
\eit
...
\parbox{\linewidth}{\footnotesize
(Sarsons, \emph{ReStud} R\&R 2025; Hackl, Hummer, and Prickner, \emph{JHlthEcon} 2015; Richards-Shubik, Roberts, and Donohue, \emph{JHlthEcon} 2022; Zeltzer, \emph{AEJ:Applied} 2020;
%Dossa, Zeltzer, Sutradhar, Simpson, and Baxter, \emph{JAMA Surgery} 2021;
%Chen, Orlas, Chang, and Kelleher, \emph{JSurgRsch} 2023;
Baker, Bundorf, and Kessler, \emph{JHlthEcon} 2016; Carlin, Feldman, and Dowd, \emph{HlthEcon} 2016)
}
%}
...
\begin{frame}{Possible excess capacity among the top 25\%}
{Totals by market x year; each specialist's capacity defined as their 75th pctile annual volume}
...
{Effect on referrals per quarter from PCP observing the failure vs.~other PCPs, with specialist/quarter FEs}
...
\begin{enumerate}
    ...
    \parbox{\linewidth}{\footnotesize (also consider PCP $i$ to specialist \textit{practice} $j$)}
    
    \medskip
    \pause

    \item Specialist (or practice) treats the patient
    \bit
        \item binary outcomes: $Y_{ijt} = 1$ (success) or 0 (failure)
        \item quality: $q_j \equiv \Pr(Y_{ijt} = 1)$,
        \emph{unknown to PCP}
    \eit 
    \medskip 
    \pause
...
\begin{frame}[t]{\only<1>{Specification (myopic version)} \only<2-4>{Identification} } 
...
\bit
    \item split fixed effects into two time periods
    \item impose $\alpha \geq 0$ via exponential transformation, $\alpha = \text{e}^{\tilde \alpha}$ (Fader et al. 1992)
\eit
...
\hline
\\
\multicolumn{8}{l}{$\alpha$ \color{gray} (utility weight on outcome, \emph{distribution across markets})} \\ 
\ \ ($\eta=1$) & 0.277 & (0.251) & 0.094 & 0.337 & & 0.371 & (0.335) & 0.131 & 0.476 \\ 
\ \ ($\eta=5$) & 0.568 & (0.548) & 0.182 & 0.746 & & 0.760 & (0.757) & 0.264 & 0.980 \\ 
\\
\multicolumn{8}{l}{$\pi$ \color{gray} (utility weight on distance, \emph{distribution across markets})} \\ 
\ \ ($\eta=1$) & -0.0694 & (0.0060) & -0.0822 & -0.0519 & & -0.0696 & (0.0058) & -0.0830 & -0.0528 \\ 
\ \ ($\eta=5$) & -0.0698 & (0.0062) & -0.0828 & -0.0523 & & -0.0693 & (0.0060) & -0.0830 & -0.0522 \\ 
\\ 
\multicolumn{8}{l}{$\gamma$ \color{gray} (congestion effect per 100 patients, \emph{single nationwide estimate})} \\  
\ \ ($\eta=1$) & -0.265 & (0.098) & & & & -0.209 & (0.068) \\ 
\ \ ($\eta=5$) & -0.577 & (0.335) & & & & -0.527 & (0.422) \\ 
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item $\gamma$ -- 100 patient increase in volume $\rightarrow$ $\approx 20\%$ relative decrease in referral prob.
\end{itemize}
...
\begin{tabular}{cc}
    ...
    \includegraphics[scale=0.2]{results/figures/fwd-timevary/Mean_Partial_Effect_Failure_FWD_eta1.png} \\
\end{tabular}
\end{center}
...
\begin{tabular}{cc}
    ...
    \includegraphics[scale=0.3]{results/figures/fwd-timevary/Reallocation_Full_FWD_eta1.png}
\end{tabular}
...
\begin{tabular}{cc}
    ...
    \includegraphics[scale=0.3]{results/figures/fwd-timevary/Mean_Health_Effect_Full_FWD_eta1.png}
\end{tabular}
...
\begin{frame}[b]{Counterfactual: specialist volume under full information} 
{Distribution across markets of change in specialist volume, as a percentage of observed volume}
...
\begin{tabular}{cc}
    Myopic model & Forward-looking model \\
    \includegraphics[scale=0.24]{results/figures/myopic-timevary/VolumeChange_Full_Myopic_eta1.png} &
    \includegraphics[scale=0.32]{results/figures/fwd-timevary/VolumeChange_Full_FWD_eta1.png}
\end{tabular}
...
\begin{itemize}
    \item Aggregate reallocation away from low volume and toward high volume specialists
    \item Some specialists effectively forced to exit entirely
\end{itemize}
...
%    \bigskip
    ...
    \item One of the first to quantify informational frictions in referrals
    ...
    \bit
        ...
        \item Small but meaningful improvements in quality
        \item Reallocation from low to high volume specialists
...
\LARGE \color{heritageblue} THANK YOU \\
\smallskip

\footnotesize ianmccarthyecon.com \\
\footnotesize ian.mccarthy@emory.edu
...
\multicolumn{4}{l}{\emph{Running values per PCP-specialist pair, over past five years:}}    \\
