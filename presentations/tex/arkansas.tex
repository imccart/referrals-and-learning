...
\item We use a structural learning model to measure the effects of uncertainty in physician referrals, and to simulate possible reallocations if there were better information about quality
...
\item Total physician/clinical services $+$ hospital care: \$2.5 trillion
...
\item Develop and estimate a structural learning model of referral decisions
% for joint replacements
\bit
    \item Prior work on health care applies learning models to prescribing decisions \  
    \hyperlink{literature}{\beamerbutton{more}}\\
...
% \item Apply to major joint replacements (hip and knee) / a specific area of health care with a well defined network of referring physicians and specialists %(joint replacement)
...
\item Find that one-third of referrals would be reallocated under full information, \\
...
\bit
    ...
    \item Some risk: 10\% complications/rehospitalized, 0.6\% die
    % from our data
\eit
\medskip
...
\bit
    ...
    % PCP: at least 20 total referrals over our study period and at least three consecutive years with observed referrals
    % Specialist: perform at least 20 surgeries per year (captures xxx\% of all surgeries)
    \item 2,241,000 surgeries, 9,250 surgeons, 51,000 PCPs in final sample
\eit
\bigskip %\pause
...
\hline
\multicolumn{4}{l}{\emph{Rate per patient/episode:}}    \\
& 90-day Complication &       0.112&       0.093&       0.101\\
& 90-day Readmission  &       0.084&       0.068&       0.075\\
& 90-day Mortality    &       0.008&       0.005&       0.006\\
& {\dots}Any Failure  &       0.114&       0.095&       0.103\\[6pt]
& Episode spending    &      23,240&      22,557&      22,839\\
&                     &    (12,717)&    (12,284)&     (12,469)\\[6pt]
\multicolumn{5}{l}{\emph{Count per doctor, per year (in sample):}}    \\
% ALT: PCP -- patients referred
& Patients referred by PCP    &       4.159&       4.740&       4.477\\
&                     &     (3.089)&     (3.611)&     (3.398)\\
& Unique specialists per PCP  &       3.002&       3.435&       3.239\\
&                     &     (1.756)&     (1.990)&     (1.900)\\[6pt]
% & Specialist -- patients treated / Patients treated by specialist xxx  &       2.666&       3.109&       2.910\\
% &                    &     (1.580)&     (1.831)&     (1.737)\\[6pt]
\multicolumn{5}{l}{\emph{Running values per PCP-specialist pair, over past five years:}}    \\
& Total Referrals     & - - -    &        4.101 & - - - \\
&                     &          &       (9.996) \\
& Failure Rate        & - - -    &       0.102  & - - - \\
&                     &          &      (0.206) \\
...
\emph{Average number of specialists per market (HRR): 70}
...
\item Reparameterize beliefs:
\bit
    \item $\rho = a_0 / (a_0 + b_0)$ \ (prior mean)
    \item $\eta = a_0 + b_0$ \ (prior strength )
\eit
\medskip

\item Other specification choices
\bit
    \item Linear effect of distance: $\pi x_{ijt}$
    \item Nonparametric familiarity: $\beta_1 I_{e = 1}$, $\beta_2 I_{e = 2}$, \dots, $\beta_K I_{e \geq 16}$
    \item Linear congestion effect, no shifters: $\gamma n_{jt}$
\eit
...
\bit
    ...
    (esp.~locations that yield the distances)
\eit
\bigskip \pause
...
\item Estimated separately in 280 geographic markets (HRRs)
\bit
...
\emph{Dep.~var.~mean = 0.194 (treated group before event), or 1.061 when $>0$}
...
{Estimated separately within 280 HRRs; statistics weighted by number of patients per HRR.}
...
\ \ ($\eta=1$) & 0.2020 & (0.1791) & 0.0000 & 0.2830 & & 0.2950 & (0.2588) & 0.0112 & 0.3954 \\ 
\ \ ($\eta=5$) & 0.3972 & (0.3723) & 0.0000 & 0.5605 & & 0.6884 & (0.6719) & 0.1307 & 0.9336 \\ 
\\
...
\\ 
\multicolumn{8}{l}{$\gamma$ \color{gray} (congestion effect per 100 patients over 3 years, \emph{single nationwide estimate})} \\  
\ \ ($\eta=1$) & -0.0913 & (0.0153) & & & & \multicolumn{2}{c}{\color{gray} -- estimation --} \\ 
\ \ ($\eta=5$) & -0.1158 & (0.0169) & & & & \multicolumn{2}{c}{\color{gray} -- in process --} \\ 
...
\begin{itemize}
    ...
    \item $\bar \alpha / \bar \pi$ -- success vs.~failure is worth 2.9 miles
    (similar to other small estimates in the literature)
    \item $\gamma$ -- 100 more patients (over 3 years) $\rightarrow$ 7-9\% relative decrease in referral probability
\end{itemize}
...
%%%
\iffalse
\begin{frame}[b]{Parameter Estimates -- familiarity xxx UPDATE} 
...
%%% 
\iffalse
\begin{frame}[t]{Partial Effect of Failures}{Distribution across markets of average partial effect, conditional on having any past referrals} 
\begin{center}
\begin{tabular}{cc}
    Myopic model & Forward-looking model \\
    \includegraphics[scale=0.22]{results/figures/myopic-timevary/Mean_Partial_FX_Failure_Myopic_eta1.png} &
    \includegraphics[scale=0.22]{results/figures/fwd-timevary/Mean_Partial_FX_Failure_FWD_eta1.png} \\
\end{tabular}
\end{center}
...
\medskip
\begin{itemize} \small
    \item Among markets where there is an effect, mean reduction in referral probability is 0.003
    \item Many markets estimated to have no response to specialist failures ($\hat \alpha = 0$)
...
\begin{frame}[t]{Counterfactual simulations}

\bigskip

Procedure:
\begin{itemize}
    \item Replace current beliefs with true success probabilities
    \item Take histories in baseline period as given (i.e., familiarity with specialists)
    % Like an information shock on January 1, 2013
    ...
    \item \textcolor{gray}{Similar results if familiarity is set to zero at beginning}
\end{itemize}
...
\begin{tabular}{cc}
    Myopic model & Forward-looking model \\
    \includegraphics[scale=0.24]{results/figures/myopic-timevary/Reallocation_Current_Myopic_eta1.png} &
    \includegraphics[scale=0.24]{results/figures/fwd-timevary/Reallocation_Current_FWD_eta1.png}
\end{tabular}
...
\begin{itemize}
    \item Averages: 39.6\% (myopic) and 32.1\% (fwd-look) are referred to a different specialist
...
\begin{tabular}{cc}
    Myopic model & Forward-looking model \\
    \includegraphics[scale=0.24]{results/figures/myopic-timevary/Mean_Health_FX_Current_Myopic_eta1.png} &
    \includegraphics[scale=0.24]{results/figures/fwd-timevary/Mean_Health_FX_Current_FWD_eta1.png}
\end{tabular}
...
\begin{itemize}
    ...
    \item About 700 fewer complications/readmissions (or deaths) per year total in sample pop.
\end{itemize}
...
\begin{itemize}
    ...
    \bit
        \item One-third of patients would be reallocated under perfect information
