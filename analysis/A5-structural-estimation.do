...
******************************************************************
**	Title:		PCP Referrals — Structural Estimation
**	Author:		Ian McCarthy
**	Date Created:	1/3/2019
**	Date Updated:	2/25/2026
**	Note:		Parameterized by MODEL_TYPE (Myopic or FWD).
**			Myopic uses posterior mean m; FWD uses Gittins index.
******************************************************************
...
if "${MODEL_TYPE}" == "Myopic" {
	...
	local belief_var "m"
	local log_label "MyopicLearning"
}
else if "${MODEL_TYPE}" == "FWD" {
	local model "FWD"
	local coef_prefix "StructureForward"
	local belief_var "gittins"
	local log_label "FWDLooking"
}
log using "${LOG_PATH}Structural_`log_label'_`logdate'.log", replace
...
forvalues i=1/457 {
	...
	if _rc==0 {
		...
		** Forward-looking: additional variables for Gittins index
		if "`model'" == "FWD" {
			gen v_git=.
			gen s_git=.
			gen psi_git=.
		...
		foreach eta in 1 5 {
			...
			** Forward-looking: compute Gittins index
			if "`model'" == "FWD" {
				local beta=0.95
			...
			** estimate and save results
			...
			if ${OUTSIDE_OPTION}==1 {
				capture cmclogit choice `belief_var' diff_dist fmly_np_* ib0.Spec_ID, noconstant basealternative(0) iterate(100)
			}
			else if ${OUTSIDE_OPTION}==0 {
				capture cmclogit choice `belief_var' diff_dist fmly_np_* ib0.Spec_ID, noconstant iterate(100)
			}
			...
			if _rc==0 {
				...
				local iter=0
				local lna_old=1
				local lna_new=ln(max(_b[`belief_var'],0.01))
				while (abs( exp(`lna_new') - exp(`lna_old') ) > 0.005 & `iter'<10) {
					local iter=`iter' + 1
					local lna_old = `lna_new'
					qui replace belief_s = exp(`lna_old')*`belief_var'
...
local r_type="${PCP_First}_${PCP_Only}_${RFR_Priority}"
if ${OUTSIDE_OPTION}==1 {
	save "${RESULTS_FINAL}`coef_prefix'HRR_Coef_`r_type'_rhobar.dta", replace
}
else if ${OUTSIDE_OPTION}==0 {
	save "${RESULTS_FINAL}`coef_prefix'HRR_CoefFull_`r_type'_rhobar.dta", replace
}
