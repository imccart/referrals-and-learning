...
log using "${LOG_PATH}CF_FullNoFE_${MODEL_TYPE}_`logdate'.log", replace

******************************************************************
**	Title:		Counterfactual — Full Info, No FEs, Reset Familiarity
**	Author:		Ian McCarthy
**	Date Created:	3/30/2026
**	Notes:		Requires base_hrr files from O2-baseline.do.
**			m = spec_qual, xi_j = 0, base_patients = 0 (converge_dyn).
******************************************************************
...
foreach eta in 1 5 {
	...
	** recreate eta-specific coefficient files for converge_dyn
	...
	forvalues h=1/`hrr_agg' {
		...
		if _rc==0 {
			...
			** simulate counterfactual — zero out FEs
			replace xi_j=0
			gen m=spec_qual
