...
sum patients if treat_group==1 & event_time<0 & patients>0
local es_base_cond = r(mean)
sum patients if treat_group==0
sum patients if event_time<0 | treat_group==0
...
local es_pct_cond: di %2.0f `es_coef'/`es_base_cond' * 100
local es_pct_cond = strtrim("`es_pct_cond'")
...
file write fh "\newcommand{\eventStudyBaseRateCond}{`=string(`es_base_cond', "%5.3f")'}" _n
file write fh "\newcommand{\eventStudyPctReduction}{`es_pct'}" _n
file write fh "\newcommand{\eventStudyPctReductionCond}{`es_pct_cond'}" _n
file close fh
