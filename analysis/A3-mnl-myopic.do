...
** Baseline with belief based on follow-up visit outcomes only
run_mnl_specs, regressors(diff_dist fmly_np_1 fmly_np_2 fmly_np_3 fmly_np_4 fmly_np_5 fmly_np_7 fmly_np_10 fmly_np_15 fmly_np_20 ib(freq).Specialist_ID) fwup
save "${RESULTS_BASE}MNL_Specs4.dta", replace
...
** baseline with fwup restriction
use "${RESULTS_BASE}MNL_Specs4.dta", clear
gen hrr=coef_val if coef_name=="hrr"
replace hrr=hrr[_n-1] if hrr==.
...
qui sum est_belief_s if spec=="fwup", detail
post table_tex (" Follow-up Visit & `=string(r(mean), "%9.4f")' & `=string(r(p10), "%9.4f")'  & `=string(r(p25), "%9.4f")'  & `=string(r(p50), "%9.4f")'  & `=string(r(p75), "%9.4f")'  & `=string(r(p90), "%9.4f")' \\")
post table_tex ("\hline\hline")
postclose table_tex
