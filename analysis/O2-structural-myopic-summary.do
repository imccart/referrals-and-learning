...
log using "${LOG_PATH}Structural_Myopic_`logdate'.log", replace

******************************************************************
**	Title:		Summarize Structural (Myopic) Results
**	Author:		Ian McCarthy
**	Date Created:	1/3/2019
**	Date Updated:	2/28/2025
******************************************************************
...
keep if converged==1
save "${RESULTS_FINAL}StructureMyopicHRR_MainCoeff_`r_type'_rhobar.dta", replace
outsheet using "${RESULTS_FINAL}StructureMyopicHRR_MainCoeff_`r_type'_rhobar.csv", comma replace
...
drop if fmly_level==.
merge m:1 hrr using hrr_size, nogenerate keep(master match)
save "${RESULTS_FINAL}StructureMyopicHRR_FmlyCoeff_`r_type'_rhobar.dta", replace
outsheet using "${RESULTS_FINAL}StructureMyopicHRR_FmlyCoeff_`r_type'_rhobar.csv", comma replace
...
if ${PCP_Practice}==1 {
	egen Spec_ID=group(Specialist_ID)
	drop Specialist_ID
	rename Spec_ID Specialist_ID
}
save "${RESULTS_FINAL}StructureMyopicHRR_Spec_FEs_`r_type'_rhobar.dta", replace
outsheet using "${RESULTS_FINAL}StructureMyopicHRR_Spec_FEs_`r_type'_rhobar.csv", comma replace
...
use "${RESULTS_FINAL}StructureMyopicHRR_Spec_FEs_1_1_0_rhobar.dta", replace
preserve
...
graph save "${RESULTS_FINAL}FE_Compare_eta1_rhobar_1_1_0", replace
graph export "${RESULTS_FINAL}FE_Compare_eta1_rhobar_1_1_0.png", as(png) replace

twoway scatter coef_val_0 coef_val_1 if eta==5 & abs(coef_val_0)<4 & abs(coef_val_1)<4, ///
	ytitle("Period One") xtitle("Period Two") color(gray) 
graph save "${RESULTS_FINAL}FE_Compare_eta5_rhobar_1_1_0", replace
graph export "${RESULTS_FINAL}FE_Compare_eta5_rhobar_1_1_0.png", as(png) replace


******************************************************************
** Summarize coefficients across HRRs

use "${RESULTS_FINAL}StructureMyopicHRR_MainCoeff_1_1_0_rhobar.dta", clear
save temp_coeff_rho, replace

use temp_coeff_rho, clear
merge m:1 hrr using temp_choice_data, nogenerate
save "${RESULTS_FINAL}StructureMyopic_SummaryHRR.dta", replace
outsheet using "${RESULTS_FINAL}StructureMyopic_SummaryHRR.csv", comma replace
...
save "${RESULTS_FINAL}StructureMyopic_Summary.dta", replace
outsheet using "${RESULTS_FINAL}StructureMyopic_Summary.csv", comma replace


** histograms of alpha (across HRRs) for given rho and eta
use "${RESULTS_FINAL}StructureMyopic_SummaryHRR.dta", replace
foreach eta in 1 5 {
	preserve
	keep if eta==`eta'
	sum coef_m, detail
	...
	graph save "${RESULTS_FINAL}alpha_eta`eta'_rhobar_1_1_0", replace
	graph export "${RESULTS_FINAL}alpha_eta`eta'_rhobar_1_1_0.png", as(png) replace		
	...
	graph save "${RESULTS_FINAL}dist_eta`eta'_rhobar_1_1_0", replace
	graph export "${RESULTS_FINAL}dist_eta`eta'_rhobar_1_1_0.png", as(png) replace		
	restore
}
...
use "${RESULTS_FINAL}StructureMyopicHRR_Spec_FEs_1_1_0_rhobar.dta", clear
...
foreach eta in 1 5 {
	...
	graph save "${RESULTS_FINAL}Structure2SLSFirstStage_eta`eta'_rhobar_1_1_0", replace
	graph export "${RESULTS_FINAL}Structure2SLSFirstStage_eta`eta'_rhobar_1_1_0.png", as(png) replace
	restore
}
...
foreach eta in 1 5 {
	...
	** hist p_val
	...
	graph save "${RESULTS_FINAL}Structure2SLSEstimate_eta`eta'_rhobar_1_1_0", replace
	graph export "${RESULTS_FINAL}Structure2SLSEstimate_eta`eta'_rhobar_1_1_0.png", as(png) replace
	restore
}
...
foreach eta in 1 5 {
	...
	use "${RESULTS_FINAL}StructureMyopicHRR_FmlyCoeff_1_1_0_rhobar.dta", clear
	keep if eta==`eta'
...
foreach eta in 1 5 {
	...
	save "${RESULTS_FINAL}MarginalEffects_Myopic`eta'.dta", replace
	erase fx_any`eta'.dta
	
	use cf_sum`eta', clear
	save "${RESULTS_FINAL}CounterFactuals_Myopic`eta'.dta", replace
	erase cf_sum`eta'.dta
	
	use cf_spec`eta', clear
	save "${RESULTS_FINAL}CounterFactualsSpec_Myopic`eta'.dta", replace
	erase cf_spec`eta'.dta	
}

foreach eta in 1 5 {
	use "${RESULTS_FINAL}MarginalEffects_Myopic`eta'.dta", clear
	merge 1:1 hrr using hrr_size, nogenerate keep(master match)
	...
	graph save "${RESULTS_FINAL}Mean_Partial_Effect_Failure_eta`eta'", replace
	graph export "${RESULTS_FINAL}Mean_Partial_Effect_Failure_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}Ptile75_Partial_Effect_Failure_eta`eta'", replace
	graph export "${RESULTS_FINAL}Ptile75_Partial_Effect_Failure_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}CS_Learning_eta`eta'", replace
	graph export "${RESULTS_FINAL}CS_Learning_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}CS_Familiarity_eta`eta'", replace
	graph export "${RESULTS_FINAL}CS_Familiarity_eta`eta'.png", as(png) replace
}

** Summarize counterfactuals: effects on reallocation and ex ante patient health

foreach eta in 1 5 {
	use "${RESULTS_FINAL}CounterFactuals_Myopic`eta'.dta", clear
	...
	graph save "${RESULTS_FINAL}Reallocation_Full_eta`eta'", replace
	graph export "${RESULTS_FINAL}Reallocation_Full_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}Reallocation_Current_eta`eta'", replace
	graph export "${RESULTS_FINAL}Reallocation_Current_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}Reallocation_FullFam_eta`eta'", replace
	graph export "${RESULTS_FINAL}Reallocation_FullFam_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}Mean_Health_Effect_Full_eta`eta'", replace
	graph export "${RESULTS_FINAL}Mean_Health_Effect_Full_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}Mean_Health_Effect_Current_eta`eta'", replace
	graph export "${RESULTS_FINAL}Mean_Health_Effect_Current_eta`eta'.png", as(png) replace
	...
	graph save "${RESULTS_FINAL}Mean_Health_Effect_FullFam_eta`eta'", replace
	graph export "${RESULTS_FINAL}Mean_Health_Effect_FullFam_eta`eta'.png", as(png) replace
...
foreach eta in 1 5 {
	...
	use "${RESULTS_FINAL}CounterFactualsSpec_Myopic`eta'.dta", clear
	...
	graph save "${RESULTS_FINAL}VolumeChange_Full_Myopic_eta`eta'", replace
	graph export "${RESULTS_FINAL}VolumeChange_Full_Myopic_eta`eta'.png", as(png) replace

	hist reldiff_fullfam if reldiff_fullfam>-1 & reldiff_fullfam<1, fraction color(gray) ///
		ytitle("Relative Frequency") xtitle("Relative Change in Patient Volume, {&eta}=`eta'") legend(off)
	graph save "${RESULTS_FINAL}VolumeChange_FullFam_Myopic_eta`eta'", replace
	graph export "${RESULTS_FINAL}VolumeChange_FullFam_Myopic_eta`eta'.png", as(png) replace
